...
void PhScatteringMatrix::builder(std::shared_ptr<VectorBTE> linewidth,
  ...
  if(!context.getElphFileName().empty()) {
    ...
    auto [elBandStructure, elStatisticsSweep] = ActiveBandStructure::builder(context, electronH0, fullPoints);
    ...
    //getLinewidths(*phelLinewidths).outputToJSON("rta_phel_relaxation_times.json", outerBandStructure);
