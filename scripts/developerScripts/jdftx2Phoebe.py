...
# we have to subtract off the nElectrons which was
# below the disentanglement window. Put that number here
nBandsExcluded = 0
nElecSubtract = nBandsExcluded
# if calclation is non-spin (numSpinPerBand = 2)
# then we multiply this by two below

phaseConvention = 1
spinPostFix = [""] # this works for no spin and SOC
...
for spin in spinPostFix:
    ...
    Wwannier = np.fromfile("wannier.mlwfCellWeights"+spin)
    ...
    Hreduced = np.fromfile("wannier.mlwfH"+spin).reshape((kfoldProd,nBands,nBands)).swapaxes(1,2)
    ...
    cellWeightsEph = np.fromfile("wannier.mlwfCellWeightsPh"+spin).reshape((nCellsEph,nBands,nAtoms)).swapaxes(1,2)
    ...
    HePhReduced = np.fromfile('wannier.mlwfHePh'+spin).reshape((prodPhononSup,prodPhononSup,nModes,nBands,nBands)).swapaxes(3,4)
...
print("Done writing to HDF5 for use in Phoebe.")
