...
std::tuple<Crystal, PhononH0> JDFTxParser::parsePhHarmonic(Context &context) {
  ...
  try {
    ...
    if (int(cellMap[0].size()) != nCells) {
      Error("Somehow, the number of R vectors in your JDFTx ph cell map does\n"
            "not match your force constant file dimensions! Check the input "
            "JDFTx files.");
    }
    if (cellMap.size() != 3) {
      Error("JDFTx ph cellMap does not have correct first dimension.");
    }

  } catch (std::exception &error) {
    if (mpi->mpiHead())
      std::cout << error.what() << std::endl;
    Error("Issue found while reading jdftx.elph.phoebe.hdf5. Make sure it "
          "exists at " +
          fileName + "\n and is not open by some other persisting processes.");
  }

#else
  Error("To use JDFTx input for phonon force constants, you must build the "
        "code with HDF5.");
#endif
      ...
      for (int ja = 0; ja < numAtoms; ja++) {
        for (int i : {0, 1, 2}) {
          for (int j : {0, 1, 2}) {
            ...
            forceConstants(i, j, ia, ja, iR) =
                FC2s[iR][ia][i][ja][j] *
                sqrt(speciesMasses(iSpecies) * speciesMasses(jSpecies));
          }
        }
      }
    }
  ...
  PhononH0 dynamicalMatrix(crystal, forceConstants, qMesh, cellMapReformat,
                           cellWeights, 2);
  ...
  if (context.getSumRuleFC2() != "none") {
    Warning("The phonon force constants from JDFTx already have a sum rule "
            "applied.\n"
            "Therefore, the sum rule chosen from input will be ignored.");
  }
  if (speciesMasses.size() > 1) {
    Warning("You have a material which has more than one species, and "
            "therefore may be polar.\n"
            "Currently, we are not set up to parse the effective charges and "
            "dielectric matrix from JDFTx --\n"
            "however this should be possible, so let us know if you need this "
            "capability.");
  }
...
JDFTxParser::parseElHarmonicWannier(Context &context) {
  ...
  try {
    ...
    // Sanity check data that has been read in
    if (int(cellMap[0].size()) != nCells) {
      Error("Somehow, the number of R vectors in your JDFTx elWannier cell map "
            "does\n"
            "not match your wannier.mlwfH file!");
    }
    if (cellMap.size() != 3) {
      Error("JDFTx el cellMap does not have correct first dimension.");
    }
...
#else
  Error("To use JDFTx input for Wannier Hamiltonian, you must build the code "
        "with HDF5.");
#endif
  ...
  // Eigen::Tensor<std::complex<double>, 4> rMatrix(3, nCells, nWannier,
  // nWannier); rMatrix.setZero();
  ...
  // copy the HWannier and cell Map into Eigen containers for the
  // ElectronWannierH0 constructor
  Eigen::MatrixXd cellMapReformat(3, nCells);
  ...
  ElectronH0Wannier electronH0(directUnitCell, cellMapReformat, cellWeights,
                               h0R);
...
Crystal JDFTxParser::parseCrystal(Context &context) {
  ...
  while (std::getline(infile, line)) {
    ...
    if (line.find("forces-output-coords") != std::string::npos) {
      ...
      while (std::getline(infile, line)) {
        ...
        tempSpecies.push_back(
            std::find(speciesNames.begin(), speciesNames.end(), pos[1]) -
            speciesNames.begin());

        // save position
        std::vector<double> temp;
        for (int j = 0; j < 3; j++)
          temp.push_back(std::stod(pos[j + 2]));
  ...
  if (context.getElectronH0Name().empty() &&
      context.getPhFC2FileName().empty()) {
    Error("Check your path, jdftx.elph.phoebe.hdf5 not found in phFC2FileName "
          "or electronH0Name.");
  }
  ...
  if (mpi->mpiHead())
    std::cout << "Reading in " + fileName +
                     " to get born charges and dielectric matrix."
              << std::endl;
  try {
